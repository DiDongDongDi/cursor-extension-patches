...
(function () {
	...
	function isMermaidBlock(codeBlock, pre) {
		const classNames = `${codeBlock.className || ''} ${pre.className || ''}`.toLowerCase();
		if (classNames.includes('language-mermaid') || classNames.includes('mermaid')) {
			return true;
		}

		const text = (codeBlock.textContent || '').trim().toLowerCase();
		return text.startsWith('flowchart ')
			...
			|| text.startsWith('<svg id="dmermaid')
			|| text.startsWith("<svg id='dmermaid");
	}
