...
### 4.1 `parser.js` — Obsidian 图片 / Excalidraw wiki 预解析

把裸图名映射到 vault 根绝对路径，**不改笔记正文**。另：Excalidraw 嵌入
`![[Drawing….excalidraw|宽]]` → 伴生 autoexport SVG `Drawing….excalidraw.svg`
（依赖 Obsidian 插件开启 **Auto-export SVG**；SVG 与源图同目录，不进 `/images/`）。

完整实现见：`~/MyNotes/.crossnote/parser.js`（快照
`workspace-config/MyNotes.crossnote/parser.js`）。
