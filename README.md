...
| 扩展 ID                                     | 类型        | 改动摘要                                                                                                                                                                                                                                                  |
| ------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shd101wyy.markdown-preview-enhanced`       | 上游 + 补丁 | 灯箱缩放/拖拽、API proxy、双击跳原文、`yAu`/`zsu`/`XBa` 打开编辑器、vue.diff 配色、已有预览再按快捷键聚焦、禁用 Cmd/Ctrl+滚轮缩放、代码块复制按钮、预览内建查找条（绕过 Cursor webview Find 失效）、关闭 md 源文件时同步关闭对应预览（当前对齐上游 **0.8.34**） |
